...
- **git 标签规范(用户指定,2026-09-22)**:标签一律 `v` 前缀 + 语义版本(`v1.0.0`/`v2.3.4`);非生产用途加预发布后缀(`v0.2.0-alpha`/`v5.9-beta.3`)。本项目现状:功能增量用四段小版本(如 `v2.3.4-command-spectrum`)——后缀命名保留,但版本段须符合 semver 形态(主.次.修订),不再使用带下划线长标签(如 `v2.3.4-command-spectrum` 属于合规边缘:带后缀可以,但正式版本号语义应在 CHANGELOG 有对应记录)
